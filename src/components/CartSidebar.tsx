...
const CartSidebar = ({ isOpen, onClose, items, onUpdateQuantity, onCheckout }: CartSidebarProps) => {
  ...
  return (
    <>
      {/* Backdrop */}
      <div 
        className="fixed inset-0 bg-black/50 z-[var(--layer-overlay)] transition-opacity duration-300"
        onClick={onClose}
      />
      
      {/* Sidebar */}
      <div className={`fixed right-0 top-0 h-full w-96 bg-card border-l shadow-2xl z-[var(--layer-overlay)] transform transition-transform duration-300 ${
